...
export default function MediaLibraryPage() {
  ...
  async function load() {
    ...
    const [{data:petData},{data:mediaData,error:mediaError}] = await Promise.all([
      ...
      supabase.from("pet_media")
        .select("id,pet_id,storage_path,original_name,mime_type,size_bytes,caption,created_at,identity_role,pets(name)")
  ...
  async function removeItem(item: MediaWithUrl) {
    if(item.identity_role === "primary"){
      setMessage("Primary identity is protected. Choose another Primary identity before deleting this photo.");
      return;
    }

    if(!window.confirm("Delete this photo from PetAlyze? This cannot be undone.")) return;
  ...
  return <AuthGuard><AppShell>
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <p className="mt-2 text-[var(--muted)]">
          Upload real pet photos and choose the best ones for each pet's Identity Set.
        </p>
      </div>

      <span className="rounded-full bg-[var(--mint)] px-4 py-2 text-sm font-bold text-[var(--green)]">
        Private storage
      </span>
    </div>

    {message&&(
      <p className="mt-5 rounded-2xl bg-[var(--cream)] p-4 text-sm">
        {message}
      </p>
    )}
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[390px_1fr]">
        ...
        <div className="mt-5 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              maxLength={500}
              placeholder="At the window, summer 2026..."
            />
          </div>

          <button
            disabled={uploading||pets.length===0}
      ...
      <div className="card p-6">
        ...
        {loading ? (
          <p className="mt-5 text-[var(--muted)]">Loading photos…</p>
        ) : items.length===0 ? (
          <div className="mt-5 rounded-2xl bg-[var(--cream)] p-6">
            <div className="text-4xl">📷</div>
            <h3 className="mt-3 text-lg font-bold">
              Add the first real pet photo
            </h3>
          </div>
        ) : (
                ...
                <div className="relative aspect-square bg-[var(--cream)]">
                  ...
                  {item.identity_role && (
                    <span className="absolute left-3 top-3 rounded-full bg-white/95 px-3 py-1 text-xs font-bold shadow">
                      🧬 {ROLE_LABELS[item.identity_role]||"Identity reference"}
                    </span>
                  )}
                </div>

                <div className="p-4">
                  <p className="text-xs font-bold uppercase tracking-[.12em] text-[var(--green)]">
                    {petName(item)}
                  </p>

                  {item.caption ? (
                    <p className="mt-2 leading-6">{item.caption}</p>
                  ) : (
                    <p className="mt-2 text-sm text-[var(--muted)]">
                      {item.original_name}
                    </p>
                  ...
                  <Link
                    href={`/ai-illustration?media_id=${encodeURIComponent(item.id)}`}
                    className="btn btn-primary mt-4 w-full"
                  >
                    ✨ Create AI Illustration
                  </Link>

                  <Link
                    href={`/pets/${item.pet_id}/identity`}
                  ...
                  <div className="mt-4 flex items-center justify-between gap-3 border-t border-[var(--line)] pt-4">
                    ...
                    {item.identity_role === "primary" ? (
                      <span
                        className="text-sm font-bold text-[var(--muted)] opacity-60"
                        title="Choose another Primary identity before deleting this photo."
                      >
                        🔒 Primary protected
                      </span>
                    ) : (
